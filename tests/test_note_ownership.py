def test_user_cannot_see_other_users_notes(client):
    # --- user A signup + login ---
    r = client.post("/users", json={"email": "a@test.com", "password": "pass1234"})
    ...
    # --- user B signup + login ---
    r = client.post("/users", json={"email": "b@test.com", "password": "pass1234"})
    assert r.status_code in (200, 201)

    r = client.post("/auth/login", json={"email": "b@test.com", "password": "pass1234"})
    assert r.status_code == 200
    ...
    r = client.get("/notes", headers=headers_b)
    assert r.status_code == 200
